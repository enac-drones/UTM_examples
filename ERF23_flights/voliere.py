...
class VolierePosition():
    def __init__(self, ac, vehicles, freq=20, server="127.0.0.1", dataport=int(1511), commandport=int(1510), vel_samples=int(4), verbose=False):
        ...
        # markerset for the specified objects (defined by regexp Ex: 'Building_*')
        self.object_name = 'Building_[8][0-9][0-9]'
        self.markerset = {}
        self.threshold = 0.1 # minimum distance to deceide is the building has moved or not
        self.refresh_period = 4  # 4s 
        # start natnet interface
        self.natnet = NatNetClient(
                server=server,
                markerSetListener=self.receiveMarkerSet,
    ...
    def is_moving(self,old_pos, new_pos):
        '''
        check if the markerset position have changed (at least one of the marker)
        '''
        try:
            for po, pn in zip(old_pos, new_pos):
                if abs(po[0]-pn[0]) > self.threshold or abs(po[1]-pn[1]) > self.threshold or abs(po[2] - pn[2]) > self.threshold:
    ...
    def get_markerset_pos(self):
        return self.markerset
    
    def receiveMarkerSet(self,name, posList):
        '''
        ...
        if re.fullmatch(self.object_name, name) is not None:
            # check if message should be sent (first time or period)
            send = False
            now = time()
            if name in self.markerset:
                dt = now - self.markerset[name]['time']
                dt_refresh = now - self.markerset[name]['time_refresh']
                if dt >= period:
                    # period elapsed, check if moved
                    self.markerset[name]['time'] = now
                    moved = self.is_moving(self.markerset[name]['pos'], posList)
                    if moved:
                        send = True
                        self.markerset[name]['pos'] = posList
                if dt_refresh >= self.refresh_period:
                    # refresh period elapsed, send anyway
                    send = True
            else:
                send = True
                self.markerset[name] = {'time_refresh': now, 'time': now, 'id': self.current_index, 'pos': posList }
            ...
            #     self.markerset[name]['hull'] = [[points[i][0], points[i][1]] for i in hull.vertices]
