...
class Logger(object):
    ...
    ################################################################################
    def set_building_hulls(self, building_hulls):
        buildings_list=[]
    ...
    def save(self, flight_type='simulation'):
        ...
        with open(os.path.dirname(os.path.abspath(__file__))+"/logs/save-"+flight_type+"-flight-"+datetime.now().strftime("%m.%d.%Y_%H.%M.%S")+".npy", 'wb') as out_file:
            np.savez(out_file, timestamps=self.timestamps, states=self.states, controls=self.controls, building_hulls=self.building_hulls)
