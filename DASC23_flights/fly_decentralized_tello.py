...
def main():
    ...
    try:
        ...
        starttime= time.time()
        time_step=-1
        update_every = 40*4 # 0.25, 0.5, 1, 2, 3, 4
        ...
        while time.time()-sim_start_time < 1800:
            ...
            if time.time()-starttime > 0.025:
                ...
                for index,vehicle in enumerate(vehicle_list):
                    ...
                    print(index, [v.position for v in vehicle.vehicle_list])
                    vehicle.vehicle_list[index].position = vehicle.position

                    if index in [0,1] and time_step%update_every == 0: # FIXME : This has to be replaced with a flag in vehicle!
                        for list_index,_ in enumerate(vehicle.vehicle_list):
