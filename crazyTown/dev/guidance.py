...
class Guidance():
    ...
    def update_waypoint_index(self, aircraft_position):
        ...
        distance = np.linalg.norm(aircraft_position[:2] - waypoint) # FIXME : checking only 2D , we may want to convert this to 3D...
    ...
    def calculate_velocity_vector(self, aircraft_position):
        ...
        waypoint = np.hstack([self.trajectory[self.waypoint_index], self.flight_altitude])  #FIXME : 2D-3D issues... 
